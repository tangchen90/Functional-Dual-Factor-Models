...
dat.train<-list()
for ( i in 1:47){
  dat.train[[i]]<-extract.ages(extract.years(jpn.data[[i]], years = 1973:2012), 0:100, combine.upper=TRUE)$rate$male
}

dat.test<-list()
for ( i in 1:47){
  dat.test[[i]]<-extract.ages(extract.years(jpn.data[[i]], years = 2013:2022), 0:100, combine.upper=TRUE)$rate$male
}
...
for (k in 1:10){
  ...
  U.pred<-cp_tnsr$U
  U.pred[[3]]<-t.forc
  Z <-.superdiagonal_tensor(num_modes = tsr.dat@num_modes, len = k, elements = cp_tnsr$lambdas)
...
for(ij in 1:year_horizon)
{
  jpn.trunc<-list()
  for ( i in 1:47){
    jpn.trunc[[i]]<-extract.ages(extract.years(jpn.data[[i]], years = 1973:(n_year+ij)), 0:100, combine.upper=TRUE)$rate$male
  }
  
  jpn.omit<-list()
  for (ik in 1:47){
    jpn.omit[[ik]]<-matrix(0, nrow(jpn.trunc[[1]]),ncol(jpn.trunc[[1]]) )
    for(i in 1:nrow(jpn.trunc[[1]])){
      for (j in 1:ncol(jpn.trunc[[1]])){
        if (jpn.trunc[[ik]][i,j]==0){
          jpn.omit[[ik]][i,j]<-0.0000001
        }else{
          jpn.omit[[ik]][i,j]<-jpn.trunc[[ik]][i,j]
        }
      }
    }
  }
  jpn.truc<-list()
  for (ik in 1:47){
    jpn.truc[[ik]]<-t(log(jpn.omit[[ik]]))
  }
  res_forc<-cp.forc(jpn.truc, year_horizon = year_horizon, years = 1973:(n_year+ij), ages = 0:100, n_pop =47, K=1)
...
cp.jpmale.me<-cbind(male_me.Hokkaido , male_me.Aomori , male_me.Iwate ,
...
cp.jpmale.mae<-cbind(male_mae.Hokkaido , male_mae.Aomori , male_mae.Iwate ,
...
cp.jpmale.rmse<-cbind(male_rmse.Hokkaido , male_rmse.Aomori , male_rmse.Iwate ,
...
colnames(cp.jpmale.me)<-c("Hokkaido", "Aomori", "Iwate", "Miyagi", "Akita", "Yamagata", "Fukushima", "Ibaraki",  "Tochigi", "Gunma", "Saitama",  
...
colnames(cp.jpmale.mae)<-c("Hokkaido", "Aomori", "Iwate", "Miyagi", "Akita", "Yamagata", "Fukushima", "Ibaraki",  "Tochigi", "Gunma", "Saitama",  
                              "Chiba", "Tokyo", "Kanagawa", "Niigata", "Toyama", "Ishikawa", "Fukui", "Yamanashi", "Nagano", "Gifu", "Shizuoka", "Aichi",    
                              "Mie", "Shiga", "Kyoto", "Osaka", "Hyogo", "Nara", "Wakayama", "Tottori", "Shimane", "Okayama", "Hiroshima", "Yamaguchi",
                              "Tokushima", "Kagawa", "Ehime", "Kochi", "Fukuoka", "Saga", "Nagasaki", "Kumamoto", "Oita", "Miyazaki", "Kagoshima", "Okinawa")
colnames(cp.jpmale.rmse)<-c("Hokkaido", "Aomori", "Iwate", "Miyagi", "Akita", "Yamagata", "Fukushima", "Ibaraki",  "Tochigi", "Gunma", "Saitama",  
                               "Chiba", "Tokyo", "Kanagawa", "Niigata", "Toyama", "Ishikawa", "Fukui", "Yamanashi", "Nagano", "Gifu", "Shizuoka", "Aichi",    
                               "Mie", "Shiga", "Kyoto", "Osaka", "Hyogo", "Nara", "Wakayama", "Tottori", "Shimane", "Okayama", "Hiroshima", "Yamaguchi",
                               "Tokushima", "Kagawa", "Ehime", "Kochi", "Fukuoka", "Saga", "Nagasaki", "Kumamoto", "Oita", "Miyazaki", "Kagoshima", "Okinawa")

write.csv(cp.jpmale.me, "cp.jpmale.me.csv")
write.csv(cp.jpmale.mae, "cp.jpmale.mae.csv")
write.csv(cp.jpmale.rmse, "cp.jpmale.rmse.csv")
