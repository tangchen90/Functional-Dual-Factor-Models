...
ffm<- function(X)
{
  ...
  for(i in 1:r){
    ...
    av_approx[i] <- 1-normi(chi_est_cum[,,i]-HDFFM.ts$coefs,ts$V)^2/normi(HDFFM.ts$coefs,HDFFM.ts$V)^2 # Estimating the average explained variance by the $i$-th compnent
  }
...
find_enlarge_val_ffm <- function(data_series, fh , nboot = 1000, alpha = 0.8, n_pop)
{
  ...
  for(ij in 1:(length(data_series[[1]]$year) - n_test - fh))
  {
    ...
    fore_res = ffmfts(dat=dat.rate.female_h_step, year_horizon = fh, n_pop = length(data_series), ages =data_series[[1]]$age)
  ...
  fore_ffm_female <- list()
  for (il in 1: n_pop){
    fore_ffm_female[[il]]<-matrix(rep(log(fore_res[(1:101)+(il-1)*101 ,fh]), nboot), nrow = 101, ncol = nboot)
  }
  ...
  boot_PI_female <- list()
  for (il in 1: n_pop){
    boot_PI_female[[il]]<- female.boot_err[[il]] + fore_ffm_female[[il]]
  }
...
interval_score_ffm <- function(PI_val, data_series, fh, alpha = 0.8)
{
  ...
  score_female  = mean((PI_ub_val_female - PI_lb_val_female) + 2/(1 - alpha) * (PI_lb_val_female - test_val_female) * lb_ind_female + 2/(1 - alpha) * (test_val_female - PI_ub_val_female) * ub_ind_female)
...
PI_fh_ffm<- function(data_series, fh, nboot = 1000)
{
  ...
  for(io in 1:(11-fh))
  {
    ...
    PI_boot_ffm_Hokkaido[,,io] = dum_pointwise$boot_sample_female[[1]]
    PI_boot_ffm_Aomori[,,io] = dum_pointwise$boot_sample_female[[2]]
    PI_boot_ffm_Iwate[,,io] = dum_pointwise$boot_sample_female[[3]]
    PI_boot_ffm_Miyagi[,,io] = dum_pointwise$boot_sample_female[[4]]
    PI_boot_ffm_Akita [,,io] = dum_pointwise$boot_sample_female[[5]]
    PI_boot_ffm_Yamagata[,,io] = dum_pointwise$boot_sample_female[[6]]
    PI_boot_ffm_Fukushima[,,io] = dum_pointwise$boot_sample_female[[7]]
    PI_boot_ffm_Ibaraki[,,io] = dum_pointwise$boot_sample_female[[8]]
    PI_boot_ffm_Tochigi[,,io] = dum_pointwise$boot_sample_female[[9]]
    PI_boot_ffm_Gunma[,,io] = dum_pointwise$boot_sample_female[[10]]
    PI_boot_ffm_Saitama[,,io] = dum_pointwise$boot_sample_female[[11]]
    PI_boot_ffm_Chiba[,,io] = dum_pointwise$boot_sample_female[[12]]
    PI_boot_ffm_Tokyo[,,io] = dum_pointwise$boot_sample_female[[13]]
    PI_boot_ffm_Kanagawa[,,io] = dum_pointwise$boot_sample_female[[14]]
    PI_boot_ffm_Niigata[,,io] = dum_pointwise$boot_sample_female[[15]]
    PI_boot_ffm_Toyama[,,io] = dum_pointwise$boot_sample_female[[16]]
    PI_boot_ffm_Ishikawa[,,io] = dum_pointwise$boot_sample_female[[17]]
    PI_boot_ffm_Fukui[,,io] = dum_pointwise$boot_sample_female[[18]]
    PI_boot_ffm_Yamanashi[,,io] = dum_pointwise$boot_sample_female[[19]]
    PI_boot_ffm_Nagano[,,io] = dum_pointwise$boot_sample_female[[20]]
    PI_boot_ffm_Gifu[,,io] = dum_pointwise$boot_sample_female[[21]]
    PI_boot_ffm_Shizuoka[,,io] = dum_pointwise$boot_sample_female[[22]]
    PI_boot_ffm_Aichi[,,io] = dum_pointwise$boot_sample_female[[23]]
    PI_boot_ffm_Mie[,,io] = dum_pointwise$boot_sample_female[[24]]
    PI_boot_ffm_Shiga[,,io] = dum_pointwise$boot_sample_female[[25]]
    PI_boot_ffm_Kyoto[,,io] = dum_pointwise$boot_sample_female[[26]]
    PI_boot_ffm_Osaka[,,io] = dum_pointwise$boot_sample_female[[27]]
    PI_boot_ffm_Hyogo[,,io] = dum_pointwise$boot_sample_female[[28]]
    PI_boot_ffm_Nara[,,io] = dum_pointwise$boot_sample_female[[29]]
    PI_boot_ffm_Wakayama[,,io] = dum_pointwise$boot_sample_female[[30]]
    PI_boot_ffm_Tottori[,,io] = dum_pointwise$boot_sample_female[[31]]
    PI_boot_ffm_Shimane[,,io] = dum_pointwise$boot_sample_female[[32]]
    PI_boot_ffm_Okayama[,,io] = dum_pointwise$boot_sample_female[[33]]
    PI_boot_ffm_Hiroshima[,,io] = dum_pointwise$boot_sample_female[[34]]
    PI_boot_ffm_Yamaguchi[,,io] = dum_pointwise$boot_sample_female[[35]]
    PI_boot_ffm_Tokushima[,,io] = dum_pointwise$boot_sample_female[[36]]
    PI_boot_ffm_Kagawa[,,io] = dum_pointwise$boot_sample_female[[37]]
    PI_boot_ffm_Ehime [,,io] = dum_pointwise$boot_sample_female[[38]]
    PI_boot_ffm_Kochi[,,io] = dum_pointwise$boot_sample_female[[39]]
    PI_boot_ffm_Fukuoka[,,io] = dum_pointwise$boot_sample_female[[40]]
    PI_boot_ffm_Saga[,,io] = dum_pointwise$boot_sample_female[[41]]
    PI_boot_ffm_Nagasaki[,,io] = dum_pointwise$boot_sample_female[[42]]
    PI_boot_ffm_Kumamoto[,,io] = dum_pointwise$boot_sample_female[[43]]
    PI_boot_ffm_Oita[,,io] = dum_pointwise$boot_sample_female[[44]]
    PI_boot_ffm_Miyazaki[,,io] = dum_pointwise$boot_sample_female[[45]]
    PI_boot_ffm_Kagoshima[,,io] = dum_pointwise$boot_sample_female[[46]]
    PI_boot_ffm_Okinawa[,,io] = dum_pointwise$boot_sample_female[[47]]
  }
...
#ffm_fh_PI = foreach(mm = 1:10, .packages = "demography") %dopar% 
  ffm_fh_PI<-list()
for(mm in 1:10){
  ffm_fh_PI[[mm]]<-PI_fh_ffm(data_series = dat.female,  fh = mm)
}
  

female_score.ffm.Hokkaido = female_score.ffm.Aomori = female_score.ffm.Iwate = female_score.ffm.Miyagi = 
  female_score.ffm.Akita = female_score.ffm.Yamagata = female_score.ffm.Fukushima = female_score.ffm.Ibaraki = 
  female_score.ffm.Tochigi = female_score.ffm.Gunma = female_score.ffm.Saitama= female_score.ffm.Chiba = 
  female_score.ffm.Tokyo= female_score.ffm.Kanagawa = female_score.ffm.Niigata = female_score.ffm.Toyama = 
  female_score.ffm.Ishikawa = female_score.ffm.Fukui = female_score.ffm.Yamanashi = female_score.ffm.Nagano = 
  female_score.ffm.Gifu = female_score.ffm.Shizuoka = female_score.ffm.Aichi = female_score.ffm.Mie = 
  female_score.ffm.Shiga = female_score.ffm.Kyoto = female_score.ffm.Osaka = female_score.ffm.Hyogo = 
  female_score.ffm.Nara = female_score.ffm.Wakayama = female_score.ffm.Tottori = female_score.ffm.Shimane = 
  female_score.ffm.Okayama = female_score.ffm.Hiroshima = female_score.ffm.Yamaguchi = female_score.ffm.Tokushima = 
  female_score.ffm.Kagawa = female_score.ffm.Ehime = female_score.ffm.Kochi = female_score.ffm.Fukuoka = 
  female_score.ffm.Saga = female_score.ffm.Nagasaki = female_score.ffm.Kumamoto = female_score.ffm.Oita =   
  female_score.ffm.Miyazaki = female_score.ffm.Kagoshima = female_score.ffm.Okinawa =vector( ,year_horizon)

for(ik in 1:year_horizon)
{
  # me 
  #### Hokkaido###
  female_score.ffm.Hokkaido[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Hokkaido,
                                                         data_series = dat.female[[1]], fh = ik)$score_female
  
  
  #### Aomori###
  female_score.ffm.Aomori[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Aomori,
                                                       data_series = dat.female[[2]], fh = ik)$score_female
  #### Iwate###
  female_score.ffm.Iwate[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Iwate,
                                                      data_series = dat.female[[3]], fh = ik)$score_female
  #### Miyagi###
  female_score.ffm.Miyagi[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Miyagi,
                                                       data_series = dat.female[[4]], fh = ik)$score_female
  #### Akita###
  female_score.ffm.Akita[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Akita,
                                                      data_series = dat.female[[5]], fh = ik)$score_female
  #### Yamagata###
  female_score.ffm.Yamagata[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Yamagata,
                                                         data_series = dat.female[[6]], fh = ik)$score_female
  #### Fukushima###
  female_score.ffm.Fukushima[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Fukushima,
                                                          data_series = dat.female[[7]], fh = ik)$score_female
  #### Ibaraki###
  female_score.ffm.Ibaraki[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Ibaraki,
                                                        data_series = dat.female[[8]], fh = ik)$score_female
  #### Tochigi###
  female_score.ffm.Tochigi[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Tochigi,
                                                        data_series = dat.female[[9]], fh = ik)$score_female
  #### Gunma###
  female_score.ffm.Gunma[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Gunma,
                                                      data_series = dat.female[[11]], fh = ik)$score_female
  #### Saitama###
  female_score.ffm.Saitama[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Saitama,
                                                        data_series = dat.female[[12]], fh = ik)$score_female
  #### Chiba###
  female_score.ffm.Chiba[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Chiba,
                                                      data_series = dat.female[[13]], fh = ik)$score_female
  #### Tokyo###
  female_score.ffm.Tokyo[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Tokyo,
                                                      data_series = dat.female[[14]], fh = ik)$score_female
  #### Kanagawa###
  female_score.ffm.Kanagawa[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Kanagawa,
                                                         data_series = dat.female[[1]], fh = ik)$score_female
  #### Niigata###
  female_score.ffm.Niigata[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Niigata,
                                                        data_series = dat.female[[15]], fh = ik)$score_female
  #### Toyama###
  female_score.ffm.Toyama[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Toyama,
                                                       data_series = dat.female[[16]], fh = ik)$score_female
  #### Ishikawa###
  female_score.ffm.Ishikawa[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Ishikawa,
                                                         data_series = dat.female[[17]], fh = ik)$score_female
  #### Fukui###
  female_score.ffm.Fukui[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Fukui,
                                                      data_series = dat.female[[18]], fh = ik)$score_female
  #### Yamanashi###
  female_score.ffm.Yamanashi[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Yamanashi,
                                                          data_series = dat.female[[19]], fh = ik)$score_female
  #### Nagano###
  female_score.ffm.Nagano[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Nagano,
                                                       data_series = dat.female[[20]], fh = ik)$score_female
  #### Gifu###
  female_score.ffm.Gifu[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Gifu,
                                                     data_series = dat.female[[21]], fh = ik)$score_female
  #### Shizuoka###
  female_score.ffm.Shizuoka[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Shizuoka,
                                                         data_series = dat.female[[22]], fh = ik)$score_female
  #### Aichi###
  female_score.ffm.Aichi[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Aichi,
                                                      data_series = dat.female[[23]], fh = ik)$score_female
  #### Mie###
  female_score.ffm.Mie[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Mie,
                                                    data_series = dat.female[[24]], fh = ik)$score_female
  #### Shiga###
  female_score.ffm.Shiga[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Shiga,
                                                      data_series = dat.female[[25]], fh = ik)$score_female
  #### Kyoto###
  female_score.ffm.Kyoto[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Kyoto,
                                                      data_series = dat.female[[26]], fh = ik)$score_female
  #### Osaka###
  female_score.ffm.Osaka[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Osaka,
                                                      data_series = dat.female[[27]], fh = ik)$score_female
  #### Hyogo###
  female_score.ffm.Hyogo[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Hyogo,
                                                      data_series = dat.female[[28]], fh = ik)$score_female
  #### Nara###
  female_score.ffm.Nara[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Nara,
                                                     data_series = dat.female[[29]], fh = ik)$score_female
  #### Wakayama###
  female_score.ffm.Wakayama[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Wakayama,
                                                         data_series = dat.female[[30]], fh = ik)$score_female
  #### Tottori###
  female_score.ffm.Tottori[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Tottori,
                                                        data_series = dat.female[[31]], fh = ik)$score_female
  #### Shimane###
  female_score.ffm.Shimane[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Shimane,
                                                        data_series = dat.female[[32]], fh = ik)$score_female
  #### Okayama###
  female_score.ffm.Okayama[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Okayama,
                                                        data_series = dat.female[[33]], fh = ik)$score_female
  #### Hiroshima###
  female_score.ffm.Hiroshima[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Hiroshima,
                                                          data_series = dat.female[[34]], fh = ik)$score_female
  #### Yamaguchi###
  female_score.ffm.Yamaguchi[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Yamaguchi,
                                                          data_series = dat.female[[35]], fh = ik)$score_female
  #### Tokushima###
  female_score.ffm.Tokushima[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Tokushima,
                                                          data_series = dat.female[[36]], fh = ik)$score_female
  #### Kagawa###
  female_score.ffm.Kagawa[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Kagawa,
                                                       data_series = dat.female[[37]], fh = ik)$score_female
  #### Ehime###
  female_score.ffm.Ehime[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Ehime,
                                                      data_series = dat.female[[38]], fh = ik)$score_female
  #### Kochi###
  female_score.ffm.Kochi[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Kochi,
                                                      data_series = dat.female[[39]], fh = ik)$score_female
  #### Fukuoka###
  female_score.ffm.Fukuoka[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Fukuoka,
                                                        data_series = dat.female[[40]], fh = ik)$score_female
  #### Saga###
  female_score.ffm.Saga[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Saga,
                                                     data_series = dat.female[[41]], fh = ik)$score_female
  #### Nagasaki###
  female_score.ffm.Nagasaki[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Nagasaki,
                                                         data_series = dat.female[[42]], fh = ik)$score_female
  #### Kumamoto###
  female_score.ffm.Kumamoto[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Kumamoto,
                                                         data_series = dat.female[[43]], fh = ik)$score_female
  #### Oita###
  female_score.ffm.Oita[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Oita,
                                                     data_series = dat.female[[44]], fh = ik)$score_female
  #### Miyazaki###
  female_score.ffm.Miyazaki[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Miyazaki,
                                                         data_series = dat.female[[45]], fh = ik)$score_female
  #### Kagoshima###
  female_score.ffm.Kagoshima[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Kagoshima,
                                                          data_series = dat.female[[46]], fh = ik)$score_female
  #### Okinawa###
  female_score.ffm.Okinawa[ik] = interval_score_ffm(PI_val = ffm_fh_PI[[ik]]$PI_boot_ffm_Okinawa,
                                                        data_series = dat.female[[47]], fh = ik)$score_female
}


ffm.jpfemale.score<-cbind(female_score.ffm.Hokkaido , female_score.ffm.Aomori , female_score.ffm.Iwate ,
                            female_score.ffm.Miyagi , female_score.ffm.Akita , female_score.ffm.Yamagata ,
                            female_score.ffm.Fukushima , female_score.ffm.Ibaraki , 
                            female_score.ffm.Tochigi , female_score.ffm.Gunma , female_score.ffm.Saitama,
                            female_score.ffm.Chiba ,  female_score.ffm.Tokyo, female_score.ffm.Kanagawa ,
                            female_score.ffm.Niigata , female_score.ffm.Toyama , female_score.ffm.Ishikawa ,
                            female_score.ffm.Fukui , female_score.ffm.Yamanashi , female_score.ffm.Nagano , 
                            female_score.ffm.Gifu , female_score.ffm.Shizuoka , female_score.ffm.Aichi ,
                            female_score.ffm.Mie ,  female_score.ffm.Shiga , female_score.ffm.Kyoto ,
                            female_score.ffm.Osaka , female_score.ffm.Hyogo ,  female_score.ffm.Nara ,
                            female_score.ffm.Wakayama , female_score.ffm.Tottori , female_score.ffm.Shimane ,
                            female_score.ffm.Okayama , female_score.ffm.Hiroshima , 
                            female_score.ffm.Yamaguchi ,  female_score.ffm.Tokushima , 
                            female_score.ffm.Kagawa , female_score.ffm.Ehime , female_score.ffm.Kochi ,
                            female_score.ffm.Fukuoka , female_score.ffm.Saga , female_score.ffm.Nagasaki ,
                            female_score.ffm.Kumamoto , female_score.ffm.Oita , female_score.ffm.Miyazaki ,
                            female_score.ffm.Kagoshima , female_score.ffm.Okinawa )

colnames(ffm.jpfemale.score)<-c("Hokkaido", "Aomori", "Iwate", "Miyagi", "Akita", "Yamagata", "Fukushima", "Ibaraki",  "Tochigi", "Gunma", "Saitama",  
...
write.csv(ffm.jpfemale.score, "ffm.jpfemale.score.csv")
